...
if [ $USE_CERT_MGR == "true" ]; then
  ...
  $KUBE_CLI apply -f $DEPLOY_PATH/role.yaml -n $TARGET_NAMESPACE
  $KUBE_CLI apply -f $DEPLOY_PATH/role_binding.yaml -n $TARGET_NAMESPACE
  ...
  $KUBE_CLI apply -f $DEPLOY_PATH/operator.yaml -n $TARGET_NAMESPACE
else
